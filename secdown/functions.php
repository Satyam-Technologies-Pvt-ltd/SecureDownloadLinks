...
/******************* Main Functions *******************/
function downloadcode($url, $pretendname = 'download', $notadl = 0 , $ext=''){ // Function to generate download code   
...
//writedberror($result_urlchk);
    ...
    addnewurltodb($url,$pretendname,$downloadpage, $notadl, $ext);
    ...
    $dbnotadl = mysql_result($result_urlchk, 0,'notadl');  
     $dbext = mysql_result($result_urlchk, 0,'fileexten');    
        ...
        if($dbnotadl!=$notadl){
        adddlstatusdb($notadl, $file_id);	
        }
        if($dbext!=$ext){
        adddfileextdb($ext, $file_id);	
        }        
...
function downloadurl($url, $pretendname = 'download', $notadl = 0, $ext=''){      //Function to print download URL in Download Page
$downloadcode = downloadcode($url, $pretendname, $notadl,$ext);
...
function addnewurltodb($url,$pretendname,$downloadpage, $notadl = 0, $ext=''){
    $query_dcins = "INSERT INTO ddown (id,actuallink, pretendname, whoreferred, notadl, fileexten) VALUES('','$url','$pretendname','$downloadpage', {$notadl}, '{$ext}')";
...
function adddlstatusdb($notadl, $file_id){
$query_chngdl = "UPDATE ddown SET notadl='{$notadl}' WHERE id = '{$file_id}'";
$result_chngdl = @mysql_query($query_chngdl);	
}

function adddfileextdb($ext, $file_id){
$query_chngdl = "UPDATE ddown SET fileexten='{$ext}' WHERE id = '{$file_id}'";
$result_chngdl = @mysql_query($query_chngdl);	
}
...
function getfiledlstatus($file_id){                                 //Get File id from actual url
$query_id = "SELECT notadl FROM ddown WHERE id = '{$file_id}' LIMIT 1";
$result_id = @mysql_query($query_id);
if(mysql_num_rows($result_id)==1){return mysql_result($result_id, 0, 'notadl');}
else return false;
}

function getfileext($file_id){                                 //Get File extension from id
$query_id = "SELECT fileexten FROM ddown WHERE id = '{$file_id}' LIMIT 1";
$result_id = mysql_query($query_id) or die(mysql_error());
if(mysql_num_rows($result_id)==1){return mysql_result($result_id, 0, 'fileexten');}
else return false;
}
...
function fileextenfrommime($mime){
	$contenttypearray = mimetypearray();
	$ext = array_search($mime, $contenttypearray);
...
function getContentType($url){
	$contenttype='';
	$contenttype = getContentTypeCurl($url);
...
function getContentTypeCurl($url){
	$ch = curl_init();
